...
RUN set -eux; \
    apt-get update; \
    apt-get install -y libwolfssl-dev wget unzip binutils build-essential; \
    cd /tmp; \
    wget https://github.com/curl/curl/releases/download/curl-8_1_2/curl-8.1.2.zip; \
    unzip curl-8.1.2.zip; \
    cd curl-8.1.2; \
    ./configure --with-wolfssl --enable-tls13 --prefix=/tmp/curl; \
    make && make install; \
    \
    rm -R /tmp/curl-8.1.2.zip && rm -R /tmp/curl-8.1.2; \
    ln -s /tmp/curl/bin/curl /usr/local/bin/curl; \
    cp /tmp/curl/lib/pkgconfig/libcurl.pc /usr/lib/pkgconfig/libcurl.pc; \
    rm -rf /var/lib/apt/lists/*; \
    \
	...
	cd /usr/src; \
	\
	curl -fsSL -k -o php.tar.xz "$PHP_URL"; \
	\
	...
	if [ -n "$PHP_ASC_URL" ]; then \
		curl -fsSL -k -o php.tar.xz.asc "$PHP_ASC_URL"; \
